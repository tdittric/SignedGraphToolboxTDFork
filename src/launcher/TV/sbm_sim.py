...
from src.launcher.TV import sbm_plotting as plotting
from src.node_classification import SpectralLearning, Sponge, TvAugmentedADMM, TvBresson, TvStandardADMM, LsbmMap, \
    DiffuseInterface
...
def get_methods(graph_config, sim_id):
    ...
    if sim_id in [0, 1]:
        for e in range(0, 45, 5):
            methods.append({'name': 'tv{e:0>2d}'.format(e=e),
                            'method': TvAugmentedADMM(num_classes=num_classes, verbosity=v, degenerate_heuristic=None,
            ...
            for x in x_range:
                methods.append({'name': 'tv{e:0>2d}_regularization{x:0>2d}'.format(e=e, x=x),
                                'method': TvAugmentedADMM(num_classes=num_classes, verbosity=v,
                                                          degenerate_heuristic='regularize', eps_rel=10 ** (-e / 10),
                                                          eps_abs=10 ** (-e / 10), regularization_x_min=x / 100,
                                                          return_min_tv=True)})
                methods.append({'name': 'tv{e:0>2d}_resampling{x:0>2d}'.format(e=e, x=x),
                                'method': TvAugmentedADMM(num_classes=num_classes, verbosity=v,
    ...
    if sim_id == 4:
        ...
        #                 'method': TvStandardADMM(num_classes=num_classes, verbosity=v, penalty_parameter=100,
        #                                          pre_iteration_version=2, t_max=1000, eps=1e-3 / np.sqrt(900 * 3),
        #                                          t_max_no_change=None, eps_inner=1e-3, t_max_inner=1000,
        #                                          backtracking_param=0, backtracking_tau_0=0.01)})
        # methods.append({'name': 'tv_nc_beta+2_pre2_sponge_asilomar', 'l_guess': 'sncSponge', 'is_unsupervised': False,
        #                 'method': TvStandardADMM(num_classes=num_classes, verbosity=v, penalty_parameter=100,
        #                                          pre_iteration_version=2, t_max=1000, eps=1e-3 / np.sqrt(900 * 3),
        #                                          t_max_no_change=None, eps_inner=1e-3, t_max_inner=1000,
        #                                          backtracking_param=0, backtracking_tau_0=0.01)})
        for b in np.logspace(0, 5, 6):
            for pre in [0, 1, 2]:
                ...
                # methods.append({'name': 'tv_nc_beta{b:0>+1d}_pre{t}_rand'.format(b=int(np.log10(b)), t=int(pre)),
    ...
    if sim_id == 5:
        ...
        for b in [100]:  # np.logspace(1,2,2):
            for l in [1]:
                pre = 2
                methods.append({'name': 'tv_nc_beta{b:0>5d}_l{l:d}_pre{t}_rand'.format(b=int(b), l=l, t=int(pre)),
                                'is_unsupervised': True,
                                'method': TvStandardADMM(num_classes=num_classes, verbosity=v, penalty_parameter=b,
                                                         laplacian_scaling=l, pre_iteration_version=pre, t_max=1000,
                                                         eps=1e-3 / np.sqrt(900 * 3), t_max_no_change=None,
                                                         eps_inner=1e-3, t_max_inner=1000, backtracking_param=0,
                                                         backtracking_tau_0=0.01)})
                methods.append({'name': 'tv_nc_beta{b:0>5d}_l{l:d}_pre{t}_sponge'.format(b=int(b), l=l, t=int(pre)),
                                'l_guess': 'sponge', 'is_unsupervised': True,
                                'method': TvStandardADMM(num_classes=num_classes, verbosity=v, penalty_parameter=b,
                                                         laplacian_scaling=l, pre_iteration_version=pre, t_max=1000,
                                                         eps=1e-3 / np.sqrt(900 * 3), t_max_no_change=None,
                                                         eps_inner=1e-3, t_max_inner=1000, backtracking_param=0,
                ...
                methods.append({'name': 'tv_nc_beta{b:0>4d}_l{l:d}_pre{t}_rand'.format(b=int(b), l=l, t=int(pre)),
                                'is_unsupervised': True,
                                'method': TvStandardADMM(num_classes=num_classes, verbosity=v, penalty_parameter=b,
                                                         laplacian_scaling=l, pre_iteration_version=pre, t_max=10000,
                                                         eps=1e-3, eps_admm=1e-5, t_max_no_change=None, eps_inner=1e-8,
                                                         t_max_inner=10000, backtracking_param=1 / 2,
                                                         backtracking_tau_0=0.01)})
                methods.append({'name': 'tv_nc_beta{b:0>4d}_l{l:d}_pre{t}_sponge'.format(b=int(b), l=l, t=int(pre)),
                                'l_guess': 'sponge', 'is_unsupervised': True,
                                'method': TvStandardADMM(num_classes=num_classes, verbosity=v, penalty_parameter=b,
                                                         laplacian_scaling=l, pre_iteration_version=pre, t_max=10000,
                                                         eps=1e-3, eps_admm=1e-5, t_max_no_change=None, eps_inner=1e-8,
            ...
            for l_guess in ['sncSponge']:
                methods.append(
                    {'name': 'tv_nc_beta{b:0>+1.1f}_{g}'.format(b=np.log10(b), g=l_guess),
                     'l_guess': l_guess, 'is_unsupervised': False,
                     'method': TvAugmentedADMM(num_classes=num_classes, verbosity=v, penalty_parameter=b,
                                               penalty_strat_threshold=float('inf'), y0=0, y1=0,
                                               min_norm=num_classes - 2)})
            methods.append(
                {'name': 'tv_nc_beta{b:0>+1.1f}_rand'.format(b=np.log10(b)),
                 'is_unsupervised': False,
                 'method': TvAugmentedADMM(num_classes=num_classes, verbosity=v, penalty_parameter=b,
                                           penalty_strat_threshold=float('inf'), y0=0, y1=0, min_norm=num_classes - 2)})
        ...
        for num_eig in [10, 20, 50, 100]:
            ...
            methods.append({'name': 'diffuseInterface_sponge{n:0>3d}'.format(n=num_eig),
                            'method': DiffuseInterface(num_classes=num_classes, verbosity=v, objective='sponge',
                                                       num_eig=num_eig)})
        methods.append({'name': 'tv_bresson', 'method': TvBresson(num_classes=num_classes, verbosity=v)})
        methods.append({'name': 'tv15_resampling05', 'method': TvAugmentedADMM(num_classes=num_classes, verbosity=v,
                                                                               degenerate_heuristic='rangapuram_resampling',
                                                                               eps_rel=10 ** (-15 / 10),
                                                                               eps_abs=10 ** (-15 / 10),
    ...
    if sim_id in [0, 4, 6, 9]:
        methods.append({'name': 'maprMinErr', 'l_guess': 'min_err',
                        'method': LsbmMap(num_classes=num_classes, verbosity=v, pi=pi, pe=pe, li=li, le=le,
                                          class_distribution=class_distribution, eps=1e-3)}, )
        methods.append({'name': 'maprMinCut', 'l_guess': 'min_cut',
                        'method': LsbmMap(num_classes=num_classes, verbosity=v, pi=pi, pe=pe, li=li, le=le,
                                          class_distribution=class_distribution, eps=1e-3)}, )
...
def run(pid, sim_id):
    ...
    if pid in [24, 34]:  # < len(sim.graph_config_list):
        ...
        x_lists = {k: v.tolist() for k, v in sim.embedding.items() if
                   k in ['tv{e:0>2d}'.format(e=e) for e in range(0, 45, 5)]}
